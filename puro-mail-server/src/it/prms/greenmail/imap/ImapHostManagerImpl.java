...
import it.prms.greenmail.store.InMemoryStore;
import it.prms.greenmail.store.MailFolder;
import it.prms.greenmail.store.Store;
 ...
 * the {@link it.prms.greenmail.store.InMemoryStore} implementation of {@link it.prms.greenmail.store.Store}.
...
public class ImapHostManagerImpl
     ...
     */
    public MailFolder getFolder(GreenMailUser user, String mailboxName) { //(*) per SMTP e POP3 è richiamato con mailbox = "INBOX"
        String name = getQualifiedMailboxName(user, mailboxName); //(*) per SMTP e POP3 name = #mail.<HASH-EMAIL>.INBOX (senza "<>")
        MailFolder folder = store.getMailbox(name);
        return (checkViewable(folder)); //(*) restituisce sempre l'oggetto MailFolder (senza fare controlli). Non sò cosa dovrebbe fare (credo verificare che l'utente abbia accesso alla mailbox)
    }
    ...
    private MailFolder checkViewable(MailFolder folder) { //(*) restituisce sempre "true" (l'oggetto MailFolder in realtà). Non sò cosa dovrebbe fare (credo verificare che l'utente abbia accesso alla mailbox)
        // TODO implement this.
        return folder;
    }
    ...
    public MailFolder getInbox(GreenMailUser user) throws FolderException {
        return getFolder(user, INBOX_NAME); //(*) il server POP3 usa sempre e solo questa funzione perchè ha solo la mailbox "INBOX"
    }

    /**
     * @see ImapHostManager#createPrivateMailAccount
     */
    public void createPrivateMailAccount(GreenMailUser user) throws FolderException { //(*) usato da pop3
     ...
     */
    public MailFolder createMailbox(GreenMailUser user, String mailboxName) //(*) usato da imap
            throws AuthorizationException, FolderException {
        String qualifiedName = getQualifiedMailboxName(user, mailboxName); //(*) qualifiedName = #mail.<HASH-EMAIL>.<mailboxName> (senza "<>")
        ...
        if (tokens.countTokens() < 2) {
            throw new FolderException("Cannot create store at namespace level."); //non puoi creare una cartella dentro a #mail, ma solo dentro a #mail.<HASH-EMAIL> e figli
        }
        ...
        while (tokens.hasMoreTokens()) { //se gli passi #mail.<HASH-EMAIL>.a.b.c lui si crea tutte le sottocartelle inesistenti
            ...
            if (child == null) {
                ...
                boolean makeSelectable = (!tokens.hasMoreTokens()); //se la cartella ha sottocartelle allora non si potrebbero salvare messaggi in questa cartella (ma solo nell'ultimo livello)
     ...
     *
     * @see it.prms.greenmail.imap.ImapHostManager#listMailboxes
     */
