 ...
 */
class SearchCommand extends SelectedStateCommand implements UidEnabledCommand { //(*) visto che non è implementato, restituisce tutti gli uid di tutti i messaggi (guarda fine pagina)
    ...
    public void doProcess(ImapRequestLineReader request,
        ...
        SearchTerm searchTerm = parser.searchTerm(request); //(*) oggetto che servirebbe per testare se un messaggio rispetta dei criteri di ricerca; qui ogni messaggio rispetta qualsiasi criterio di ricerca
        parser.endLine(request);

        MailFolder folder = session.getSelected();
        long[] uids = folder.search(searchTerm); //(*) restituisce gli uid dei messaggi che rispettano certi criteri di ricerca, visto che non è implementato li restituisce TUTTI
    ...
    private class SearchCommandParser extends CommandParser {
        ...
        public SearchTerm searchTerm(ImapRequestLineReader request)
            ...
            return new SearchTerm() { //(*) visto che SearchTerm è una classe astratta, crea on the fly una nuova classe che implementa SearchTerm
                public boolean match(Message message) { //(*) metodo che servirebbe per capire se un messaggio rispetta determinati criteri di ricerca; qui restituisce sempre true
                    return true;
                }
            };
        }
