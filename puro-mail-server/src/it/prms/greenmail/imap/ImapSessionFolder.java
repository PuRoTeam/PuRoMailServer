...
import it.prms.greenmail.imap.commands.IdRange;
import it.prms.greenmail.mail.MovingMessage;
import it.prms.greenmail.store.FolderException;
import it.prms.greenmail.store.FolderListener;
import it.prms.greenmail.store.MailFolder;
import it.prms.greenmail.store.SimpleStoredMessage;
...
public class ImapSessionFolder implements MailFolder, FolderListener {  //(*)  poichè più utenti possono usare in parallelo lo stesso account, se un evento causato da uno può condizionare gli altri, allora lo devo segnalare. Posso continuare a gestire tutto in memoria
    ...
    public int getMsn(long uid) throws FolderException {
        ...
        for (int i = 0; i < uids.length; i++) {
            ...
            if (uid == messageUid) {
                return i + 1; //(*) i + 1 perchè il Message Sequence Number parte da 1, mentre l'indice in un ArrayList parte da 0
            }
        }
    ...
    public void added(int msn) { //(*) ricarica magari la cartella da dynamodb in questa situazione
        _sizeChanged = true;
    }
