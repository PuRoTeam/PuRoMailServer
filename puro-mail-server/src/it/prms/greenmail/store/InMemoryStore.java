 ...
 */
public class InMemoryStore //(*) interessante notare quanto scritto sopra: "Note: this implementation does not persist *anything* to disk."
        implements Store, ImapConstants {
    private RootFolder rootMailbox = new RootFolder(); //(*) un oggetto HierarchicalFolder con nome "#mail" e senza genitore
    ...
    public MailFolder getMailbox(String absoluteMailboxName) { //(*) (IN SMTP) richiamato da ImapHostManagerImpl con parametro absoluteMailboxName = "#mail.<HASH-EMAIL>.inbox"
    ...
    /**
     * @see it.prms.greenmail.store.Store#listMailboxes
     */
    ...
    private class RootFolder extends HierarchicalFolder { //(*) un oggetto HierarchicalFolder con nome "#mail" e senza genitore
        public RootFolder() {
    ...
    private class HierarchicalFolder implements MailFolder {
        private Collection children; //(*) collezione (ArrayList) di figli 
        private HierarchicalFolder parent; //(*) genitore cartella

        protected String name;
        private boolean isSelectable = false;

        private List mailMessages = Collections.synchronizedList(new LinkedList()); //(*) insieme messaggi della Folder
        private long nextUid = 1; //(*) ogni messaggio all'interno della STESSA Folder ha un identificativo univoco (ma non fra Folder differenti)
        private long uidValidity;

      //(*) _mailboxListeners lo devo continuare a gestire in memoria, anche nella implementazione con DynamoDB
        private List _mailboxListeners = Collections.synchronizedList(new LinkedList()); //(*) Collezione di listener della Folder, gli vengono segnalati eventi come l'arrivo di nuovi messaggi (guarda ImapSessionFolder)
        ...
        public long appendMessage(MimeMessage message,
                                  Flags flags,
                                  Date internalDate) { //(*) crea un nuovo messaggio di tipo StoredMessage e lo salva nella Folder segnalando l'evento a tutti i listener registrati
            long uid = nextUid;
            nextUid++;
...
//            flags.setRecent(true);
            ...
            try {
				System.out.println("append subject: " + message.getSubject());
            ...
            synchronized (_mailboxListeners) {
                for (int j = 0; j < _mailboxListeners.size(); j++) {
                    FolderListener listener = (FolderListener) _mailboxListeners.get(j);//(*) FolderListener implementato da ImapSessionFolder
                    listener.added(newMsn); //(*) modifica questa funzione, ricaricando magari la cartella
                }
            }
        ...
        public void store(MovingMessage mail) throws Exception { //(*) richiamato da deliver(MovingMessage msg)
            store(mail.getMessage());
        }


        public void store(MimeMessage message) throws Exception { //(*) richiamato da deliver(MimeMessage msg) e da HierarchicalFolder.store(MovineMessage) (funzione sopra)
